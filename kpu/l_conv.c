...
void conv_init(kpu_task_t *task, convolution_kernel_t conv_x_x, float *conv_data)
{
	...
	la.kernel_calc_type_cfg.data.row_switch_addr = (CAM_WIDTH_PIXEL + 63) / 64; //图像宽度占用的单元数
	...
	//dma设置，知道是输出数据使用的DMA
	la.dma_parameter.data.dma_total_byte = CAM_WIDTH_PIXEL * CAM_HIGHT_PIXEL * ch_out - 1; //总共的DMA传输数量
	la.dma_parameter.data.send_data_out = 1;				   //使能数据的dma输出
	la.dma_parameter.data.channel_byte_num = CAM_WIDTH_PIXEL * CAM_HIGHT_PIXEL - 1;		   //单通道的DMA传输数量
	//卷积运算参数设置
	...
	//图像尺寸设置
	la.image_size.data.i_row_wid = CAM_WIDTH_PIXEL - 1; //输入长宽
	la.image_size.data.i_col_high = CAM_HIGHT_PIXEL - 1;
	la.image_size.data.o_row_wid = CAM_WIDTH_PIXEL - 1; //输出长宽
	...
	//批归一化首地址
	la.kernel_pool_type_cfg.data.first_stride = CAM_HIGHT_PIXEL < 256 ? 0 : 1; //图像高度未超过255
	//图像通道设置
