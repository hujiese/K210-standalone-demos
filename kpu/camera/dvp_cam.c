...
static int on_irq_dvp(void *ctx)
{
    ...
    {
        // dvp_set_display_addr((uint32_t)g_display_buf_addr);
...
{  
    ...
    /* 设置DVP的显示地址参数和关闭中断 */
    g_display_buf = (uint32_t*)iomem_malloc(CAM_WIDTH_PIXEL * CAM_HIGHT_PIXEL * 2);
    g_display_buf_addr = g_display_buf;
    // dvp_set_display_addr((uint32_t)g_display_buf_addr);
